        ...
        <section className="w-full h-[100dvh] justify-items-center sm:py-32 py-16 relative">
            ...
            <Swiper direction="vertical" slidesPerView={1} spaceBetween={window.innerWidth > 425 ? 10 : 55} modules={[Autoplay]} autoplay={isPlaying ? { delay: 1000 } : false} speed={1000}
                ...
                {showStar && projects.map((project, index) => (
                    <SwiperSlide >
                        <motion.div className={`md:flex md:flex-row flex-col w-full h-full items-center justify-center transition-all`} key={index}
                            ...
                            <motion.svg className={`animate-spin-slow transition-all duration-500 size-[30px] fill-current dark:text-white text-yellow-400 sm:mx-0 mx-auto sm:my-0 my-4 sm:w-[15vw] hover:drop-shadow-[0_0_30px_rgba(255,255,255,0.7)]`} viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <div onMouseEnter={() => setHover("description")} onMouseLeave={() => setHover("")}
                                className="dark:bg-[#111111] bg-white dark:border-none border-1 border-black flex flex-col xl:p-8 p-5 rounded-xl sm:h-[53%] h-[45%] sm:w-[45vw] sm:m-0 m-4  relative sm:mr-8 hover:scale-105 transition-all" >
